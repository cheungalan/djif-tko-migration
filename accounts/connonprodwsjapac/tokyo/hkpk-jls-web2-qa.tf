...
resource "aws_security_group" "hkpk-jls-web2-qa" {
  ...
  egress {
    description     = "MSSQL access to jls-wrfeed-qa"
    from_port       = 1433
    to_port         = 1433
    protocol        = "tcp"
...
resource "aws_instance" "hkpk-jls-web2-qa" {
  count                  = 1
  ami                    = data.aws_ami.hkpk-jls-web2-qa.image_id
